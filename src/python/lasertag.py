...
# 5% drive is deadband
deadband = 50 #0.2 * MAX_MOTOR_SPEED
...
##            print 'block: x: [%d] y: [%d] width: [%d] height: [%d] angle: [%d]' % (blocks[0].x, blocks[0].y, blocks[0].width, blocks[0].height, blocks[0].angle)
##            time.sleep(1)
...
##            panError = PIXY_X_CENTER - blocks[0].x
            wallDist = 0.005*blocks[0].y*blocks[0].y - 1.446*blocks[0].y +107.983
            throttle = 0.5     #org value = 0.1

            df = 1 / (1 + math.exp(baseDist - wallDist) / 10)   #can adjust 10 num beta for logit func

            print 'df: ', df
            # amount of steering depends on how much deviation is there
##            diffDrive = diffGain * abs(float(panError)) / PIXY_X_CENTER
            #diffDrive = 1.0/(float(wallDist-distThre))
            #diffDrive = 0.1 / (0.1 + df)
            ...
            #distError = wallDist - distThre
            # this is in float format with sign indicating advancing or retreating
            # advance = driveGain * float(distError) / refDist
            speedgain = 100
            advance = driveGain * df * speedgain     #can adjust this speedgain
            print advance
            if wallDist < distThre:
                advance = -advance
            
            print 'walldistance: [%d] advance: [%d] diffDrive: [%f]' %(wallDist, advance, diffDrive)
## circle when it is close to the wall
##            if wallDist < 12:
##                throttle = 0.2
##                diffDrive = 0.3
##                advance =1
...
def drive():
    ...
    #print 'Bias:', bias, 'Throttle', throttle, 'SynDrive', synDrive
    #print 'Left diff:', leftDiff, 'Right diff:', rightDiff

    # construct the drive levels
    #LDrive = (synDrive + leftDiff)
    #RDrive = (synDrive + rightDiff)
    #print 'L speed', int(LDrive), 'R speed', int(RDrive)
    ...
    #time.sleep(1)
    #turnLeft(int(LDrive), r)


def turnLeft(speed, r):
    RDrive = speed
    LDrive = speed * r
    if LDrive < deadband:
        LDrive = deadband
        RDrive = LDrive / r
        if RDrive > MAX_MOTOR_SPEED / 2:
            RDrive = MAX_MOTOR_SPEED / 2
            LDrive /= 2
    print 'L speed', int(LDrive), 'R speed', int(RDrive)
    motors.setSpeeds(int(LDrive), int(RDrive))
